...
function processAndSortActions(actions) {
  var acts = actions.tweets;
  acts.sort(function(a, b) {
      return b.date - a.date;
  });

  logger.debug(sys.inspect(acts));
  var today = new Date(),
    start = new Date(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate());
  var result = [], items = [];
  ...
  logger.debug(sys.inspect(result));
  return result;
}
          ...
          userProvider.getUserByCredentials(req, creds, function(error, user) {
            if (error) {
              ...
              res.render("home.ejs", {
                locals : {
                  user : user,
                  actions : processAndSortActions(user.actions)
                }
              });
            }
            else {
